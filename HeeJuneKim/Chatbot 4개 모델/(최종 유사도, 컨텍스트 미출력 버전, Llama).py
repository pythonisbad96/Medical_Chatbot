# -*- coding: utf-8 -*-
"""
medical_main.py (V6.0: OpenRouter(Llama 3.3)로 LLM 교체)
- 터미널에 출력되던 1차 검색 결과(유사도)와 LLM 컨텍스트 미리보기를 제거.
- 사용자에게는 AI의 질문과 최종 답변만 보이도록 유지.
- LLM 클라이언트: OpenRouter를 통해 Meta Llama 3.3 모델 사용.
"""
...
from openai import OpenAI           # OpenAI 클라이언트 (OpenRouter와 호환)
...
# 3) 임베딩 모델 준비
# ------------------------------------------------------------
# kosimcse를 사용해보고 싶다는 이전 요청을 기억하여 추천해 드립니다.
...
# ------------------------------------------------------------
# 4) OpenRouter LLM 클라이언트 준비
# ------------------------------------------------------------
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY")
if not OPENROUTER_API_KEY:
    # 실행 전 .env에 OPENROUTER_API_KEY=... 추가 또는 OS 환경변수 등록 필요
    raise RuntimeError("환경변수 OPENROUTER_API_KEY 이(가) 비었습니다. .env 또는 OS 환경변수에 설정하세요.")

LLM_MODEL_NAME = os.getenv("LLM_MODEL_NAME", "meta-llama/llama-3.3-70b-instruct").strip()
...
def chat_with_llm(messages: List[dict], temperature: float = 0.3, max_tokens: int = 2048) -> str:
    """
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not docs_with_scores:
            ...
            try:
                answer = chat_with_llm(general_messages)
                print(f"\n🧾 [AI 답변]\n{answer}")
        ...
        if final_docs:
            ...
            # OpenAI/OpenRouter 형식에 맞게 메시지 순서 조정
            # System -> User(Context) -> User(Query)
            messages = [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "user", "content": f"[질병 정보]\n{final_context[:CTX_CHARS]}\n\n위 정보를 바탕으로 다음 질문에 답변해 주세요:\n{final_user_input}"},
            ]
