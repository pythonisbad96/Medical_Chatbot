...
# 0) API 설정
# 기본 모델을 llama-3.5-70b-versatile로 변경
# AX4로 쓰려면 환경변수 MODEL_NAME=ax4 로 설정
...
# 5) 각 데이터셋을 FAISS 벡터DB로 저장(최초 1회)
def prepare_faiss():
...
# 7) 대화 세션 관리용 클래스 (대화내역/history 저장)
class ChatSession:
...
답변에는 **영어나 한자를 단 한 글자도 사용하지 마세요**. 모든 내용은 반드시 한글로만 작성해야 합니다.
...
def remove_non_korean(text):
    # 한글, 숫자, 공백, 필수 문장 부호(,.?!-:)만 남기고 모두 제거
    return re.sub(r"[^가-힣ㄱ-ㅎㅏ-ㅣ0-9\s.,?!-~]", "", text)


# 10) OpenAI(AX4/llama 등) 클라이언트
...
while True:
    ...
    # OpenAI ChatCompletions 스트리밍 호출
    ...
    # 한국어만 남기는 후처리 함수 추가
    answer = remove_non_korean(answer)
