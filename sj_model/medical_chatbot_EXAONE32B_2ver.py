...
# 환경변수에서 EXAONE 32B용 API 정보 설정 (직접 입력으로 변경)
OPENAI_BASE_URL = "https://api.friendli.ai/serverless/v1"
...
절대 한국어만 사용하세요. 필요한 경우(약의 단위 등)에만 예외적으로 영어를 허용합니다.
...
모든 답변은 반드시 존댓말(-입니다, -합니다, -해야 합니다 등)로만 작성하세요.
절대 불필요한 영어 단어를 사용하지 마세요. 필요한 경우(약의 단위 등)에만 예외적으로 허용합니다.
아래 [증상 정보]를 참고하여 **상비약 정보만** 출력하세요.
**최소 3개, 최대 5개**의 서로 다른 제품을 추천합니다. 각 제품은 아래 형식의 블록으로 **연속**해서 출력하고,
제품 간에는 **빈 줄 1줄**만 두세요. 표는 사용하지 않습니다. 동일/유사 성분의 중복 제품은 피하세요.
...
# 존댓말 및 불필요한 영단어 자동 후처리 함수 (최종 보강 버전)
def post_process_answer(text):
    ...
    text = re.sub(r"아오", "아요", text)

    # 불필요한 영단어 대체 및 제거
    text = re.sub(r"Elephant", "어려움", text, flags=re.IGNORECASE)
    text = re.sub(r"Premier", "건강하게", text, flags=re.IGNORECASE)
    text = re.sub(r"Solution", "해결책", text, flags=re.IGNORECASE)
    text = re.sub(r"Community", "지역사회", text, flags=re.IGNORECASE)
    text = re.sub(r"System", "시스템", text, flags=re.IGNORECASE)
    text = re.sub(r"Service", "서비스", text, flags=re.IGNORECASE)
    text = re.sub(r"Care", "관리", text, flags=re.IGNORECASE)
    text = re.sub(r"Medical", "의료", text, flags=re.IGNORECASE)

    # 최근 발견된 불필요한 단어 제거 (단어가 아닌 문자열일 가능성이 높음)
    text = re.sub(r"ORUSly", "", text, flags=re.IGNORECASE)
    text = re.sub(r"abr", "", text, flags=re.IGNORECASE)
    text = re.sub(r"Congregated", "", text, flags=re.IGNORECASE)

    # 일부 영어 단어를 한글로 대체
    text = re.sub(r"Pine", "파인", text, flags=re.IGNORECASE)

    # 추가 보강: 약품명이나 단위와 관련 없는 2글자 이상의 영단어 제거
    # 이 정규식은 mg, ml, g, unit, % 와 같은 단위를 제외한 영단어를 제거합니다.
    # 단, 약품명 전체를 제거할 수 있으므로 주의해서 사용해야 합니다.
    text = re.sub(
        r"\b(?!mg|ml|g|mcg|unit|%|etc|vs|and|or|vs)\b[A-Za-z]{2,}\b", "", text
    )
    text = re.sub(r"\s{2,}", " ", text)  # 여러 공백을 하나로 줄이기
    text = text.strip()
...
while True:
    ...
    # 2. 특정 키워드에 따라 상비약 모드 강제 전환
    is_medicine_query = (
        "상비약" in user_question or "약" in user_question or "제품" in user_question
    )
    ...
    medicine_intent = (
        is_medicine_query
        or (mfds_hits_in_topM >= 3)
        or (mfds_min + MARGIN <= other_min)
    )
    ...
    def _push_from_scored(scored_list, limit):
    ...
    retrieved_context = (
        "\n---\n".join(context_candidates)[:1600] if context_candidates else ""
    )
    ...
    # 후처리: 상비약 모드/일반 모드/비의료 모드 분리
    if medicine_intent:
        # 상비약 전용 출력 후처리
        answer_only = post_process_answer(answer)
    elif re.search(r"1\..*?\n2\..*?", answer, re.DOTALL):
        # 1~5번 항목만 추출
        matches = re.findall(
            r"1\..*?\n2\..*?\n3\..*?\n4\..*?\n5\..*(?=\n|$)",
        ...
        # 존댓말 후처리 + 다중라인
        answer_only = post_process_answer(answer_only)
        answer_only = format_multiline_sections(answer_only)
    else:
        # 비의료 질문의 경우, 원본 답변을 그대로 사용하고 후처리만 적용
        answer_only = post_process_answer(answer)
